...
Usage: python sessions/scripts/index_secondary.py s2.5 --source-class ai-summary
"""
...
def index_secondary(session_id, source_class, out_dir=None, raw_path=None,
    ...
    raw_path = raw_path or os.path.join(
        base_dir, "transcripts", "raw", f"{session_id}-raw.md")
    if not os.path.exists(raw_path):
        # survey sessions may live in clean/ directly
        alt = os.path.join(base_dir, "transcripts", "clean",
    ...
    out_dir = out_dir or os.path.join(base_dir, "transcripts", "index")
