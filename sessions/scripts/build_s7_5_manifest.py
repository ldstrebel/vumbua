...
def main():
    raw_path = "sessions/transcripts/index/s7.5-raw-indexed.md"
    sha = hashlib.sha256(open(raw_path, "rb").read()).hexdigest()
    total_lines = sum(1 for _ in open(raw_path, "r", encoding="utf-8"))

    manifest = {
        "session_id": "s7.5",
        "raw_file": "sessions/transcripts/index/s7.5-raw-indexed.md",
        "raw_file_hash": sha,
    ...
    out_path = "sessions/transcripts/index/s7.5-manifest.json"
