"""Generates sessions/data/index/s8-manifest.json and validates it."""
...
def build_manifest():
    ...
    indexed_path = os.path.join(base_dir, "data", "index", "s8-raw-indexed.md")
    manifest_path = os.path.join(base_dir, "data", "index", "s8-manifest.json")
    attr_path = os.path.join(base_dir, "data", "index", "s8-attribution.json")
    ...
    # Split lines 1 to 305 into 3 OOC chunks <= 105 lines each
    ooc_chunks = [
        (1, 100), (101, 200), (201, 305)
    ]
    ...
    for s, e in ooc_chunks:
        scene_defs.append({
            "id": sc_id,
            "title": f"OOC Pre-Session Setup & Recap Part {sc_id}",
            "range": [s, e],
            "ooc": True
        })
        sc_id += 1
    ...
    in_game_chunks = [
        ("Post-Zephyr Courtyard — Britt's Sickness", 306, 440),
        ("Friday Morning — The Infirmary", 441, 560),
        ("Dorm Rest & The Search for Lucky", 561, 700),
        ("Campus Harbor Dive & Pebble Signal", 701, 845),
        ("Apex Arena & The Written Exam", 846, 945),
        ("The Crimson Slates & Squad 907", 946, 1008),
    ]
    ...
    manifest = {
        "session_id": "s8",
        "raw_file": "data/index/s8-raw-indexed.md",
