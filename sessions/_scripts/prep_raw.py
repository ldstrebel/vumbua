...
def prep_raw(session_id, out_dir=None, config_path=None, sessions_dir=None):
    ...
    raw_path = os.path.join(base_dir, "data", "raw", f"{session_id}-raw.md")
    index_dir = out_dir or os.path.join(base_dir, "data", "index")
...
def main(argv=None):
    ...
    parser.add_argument(
        "--out-dir",
        dest="out_dir",
        help="where to write sN-raw-indexed.md (default sessions/data/index)",
    )
