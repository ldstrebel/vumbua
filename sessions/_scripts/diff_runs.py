...
Produces a structured Markdown diff report in sessions/_compare/{session_id}-diff-report.md.
...
def run_diff(session_id):
    base_dir = "sessions"
    compare_dir = os.path.join(base_dir, "_compare")
    ...
    ag_decisions_path = os.path.join(base_dir, "data", "index", f"{session_id}-attribution-decisions.json")
    ag_manifest_path = os.path.join(base_dir, "data", "index", f"{session_id}-manifest.json")
    ag_story_path = os.path.join(base_dir, "data", "clean", f"{session_id}-clean-story.md")
    ag_assumptions_path = os.path.join(base_dir, "data", "index", f"{session_id}-assumptions.json")
    ...
    lines.append("1. **Strict Gate Validation Confirmed:** `s0-session-config.json` has been updated to use the canonical object schema (`{'person': 'Kristina', 'identity': 'Kristina', 'kind': 'player_character'}`) with self-mapped player identities for Session 0, passing `python sessions/_scripts/attribute_speakers.py s0 --strict` with zero violations.")
